...
void alloc_pattern(size_t size_min, size_t size_max)
{
    ...
    std::cout <<  "Total allocated size " << total_alloc_size << std::endl;
    std::cout <<  "Total allocation no " << pmem_vec.size() << std::endl;
    std::cout <<  std::endl;
...
void alloc_pattern_2(void)
{
    const size_t alloc_no = 1000000000;
    size_t total_alloc_size = 0;
    std::random_device rd; // obtain a random number from hardware
    std::mt19937 gen(rd()); // seed the generator

    for (size_t i = 0; i< alloc_no ;i++) {
        void *ptr = memkind_malloc(pmem_kind, SIZE_1);
        if (!ptr)
            goto status;
        memset(ptr, 'a', SIZE_1);
        total_alloc_size += memkind_malloc_usable_size(pmem_kind, ptr);
    ...
    for (size_t i = 0; i< alloc_no ;i++) {
        void *ptr = memkind_malloc(pmem_kind, SIZE_2);
        if (!ptr)
            goto status;
        memset(ptr, 'a', SIZE_2);
        total_alloc_size += memkind_malloc_usable_size(pmem_kind, ptr);
