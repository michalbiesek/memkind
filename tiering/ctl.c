...
{
    memkind_t *new_kinds = memkind_realloc(MEMKIND_DEFAULT, fs_dax_reg_g.kinds, sizeof(fs_dax_reg_g.kinds) * (fs_dax_reg_g.size + 1));
    if (!new_kinds)
        return -1;
...
static memkind_t ctl_get_kind(const ctl_tier_cfg *tier)
{
    ...
    if (strcmp(tier->kind_name, "DRAM") == 0) {
        ...
        if (kind) {
            // dummy allocation for initialize arena
