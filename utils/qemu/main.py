...
QemuCfg = collections.namedtuple('QemuCfg', ['workdir', 'image', 'interactive', 'force_reinstall', 'run_test', 'topology_l'])
...
class MemoryHostException(Exception):
    """
    Exception raised for errors in case of unsufficient memory on host.
    """
    ...
    def __str__(self) -> str:
        return f'Memory on host {self.total_memory} bytes are not enough for memory required by {self.tpg} : {self.tpg_memory} bytes.'
...
class GuestConnection():
    ...
    def _build_and_reinstall_memkind(self, c: fabric.Connection) -> None:
        ...
        """
        # TODO try handling this by nproc and ccache
    ...
    def _run_memkind_test(self, c: fabric.Connection) -> None:
        ...
        Run memkind test
        """
        c.run('echo $TOPOLOGY_ENV_VAR', echo=True)
    ...
    def _shutdown(self, c: fabric.Connection) -> None:
        ...
        Shutdown guest
        """
        c.run('sudo shutdown now', echo=True)
    ...
    def run_connection_test(self, force_reinstall: bool) -> None:
        ...
        with fabric.Connection(**self._connection_params, inline_ssh_env=True) as c:
            try:
                ...
                self._shutdown(c)
                # TODO wait for shutdown QEMU - check if there is better way
                time.sleep(5)
        ...
        with fabric.Connection(**self._connection_params, inline_ssh_env=True) as c:
            try:
                with c.cd(MEMKIND_GUEST_PATH):
                    ...
                    if force_reinstall:
                        self._build_and_reinstall_memkind(c)
                print("QEMU development is ready to use via SSH/VNC")
...
class QEMU():
    ...
    def _start_qemu_proc(self, qemu_cmd: str) -> None:
        """
        Start QEMU process
        """
        try:
    ...
    def run(self) -> None:
        ...
        for tpg in self.cfg.topology_l:
            try:
                qemu_cmd = self._qemu_cmd(tpg)
            except MemoryHostException as e:
                print(f' QEMU ERROR: Memory topology skipped {tpg.name}')
                print(f'{e}')
                continue
            if self.cfg.interactive:
                ...
                self._start_qemu_proc(qemu_cmd)
                time.sleep(10)
                con = GuestConnection(self.cfg, tpg.name)
...
def parse_topology_xml(tpg_file_name: str) -> TopologyCfg:
    ...
    """
    try:
        tree = ET.parse(tpg_file_name)
        root = tree.getroot()
        xmlstr = ET.tostring(root, encoding='unicode', method='xml')
        conn = libvirt.open(None)
        libvirt_args = conn.domainXMLToNative('qemu-argv', xmlstr)

        tpg_cfg = {'name':  re.search(r'guest=(.\w+)', libvirt_args).group(1)}
        tpg_cfg['hmat'] = True if libvirt_args.find('hmat=on') != -1 else False
        tpg_cfg['cpu_options'] = re.search('(?=-smp)(.*)(?=threads=[0-9]+)', libvirt_args).group(1)
        tpg_cfg['mem_options'] = re.search('(?=-object memory)(.*)(?=-uuid)', libvirt_args).group(1)

        tpg = TopologyCfg(**tpg_cfg)
    except FileNotFoundError:
        sys.exit(f'\n XML file: {tpg_file_name} is missing.')
    except libvirt.libvirtError:
        sys.exit(f'\n XML file: {tpg_file_name} error in libvirt parsing')
    return tpg
...
def parse_topology(cfg: dict) -> list:
    ...
    """
    tpg_list = []
    single_top = cfg.get('topology', None)
    if single_top:
        tpg_list.append(parse_topology_xml(single_top))
    elif cfg['mode'] == 'test':
        topology_dir = os.path.join(find_root_memkind_dir(), MEMKIND_QEMU_PREFIX, TOPOLOGY_DIR)
        for filename in os.listdir(topology_dir):
            if filename.endswith(".xml"):
                filename_with_path = os.path.join(topology_dir, filename)
                tpg_list.append(parse_topology_xml(filename_with_path))
    else:
        DevTopology = {'name': '', 'hmat': False, 'cpu_options': '', 'mem_options': ''}
        tpg_list.append(TopologyCfg(**DevTopology))
    return tpg_list
...
def parse_arguments() -> QemuCfg:
    ...
    qemu_cfg['run_test'] = True if cli_cfg['mode'] == 'test' else False
