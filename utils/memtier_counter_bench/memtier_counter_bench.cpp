...
protected:
    void *bench_alloc(size_t size) const final
    {
        return memtier_tier_malloc(m_tier, size);
    }

    void bench_free(void *ptr) const final
    {
        memtier_tier_realloc(m_tier, ptr, 0);
    }
private:
...
public:
    memtier_bench_alloc()
    {
        ...
        memtier_builder_add_tier(m_tier_builder, m_tier, 1);
        memtier_builder_set_policy(m_tier_builder, MEMTIER_POLICY_STATIC_THRESHOLD);
        memtier_builder_construct_kind(m_tier_builder, &m_tier_kind);
    }
...
protected:
    void *bench_alloc(size_t size) const final
    {
        return memtier_kind_malloc(m_tier_kind, size);
    }

    void bench_free(void *ptr) const final
    {
        memtier_kind_realloc(m_tier_kind, ptr, 0);
    }
