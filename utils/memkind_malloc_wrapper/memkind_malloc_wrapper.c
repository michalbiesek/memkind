...
MEMKIND_WRAPPER_EXPORT void memkind_malloc_post(memkind_t kind, size_t size, void **result)
{
...
MEMKIND_WRAPPER_EXPORT void memkind_calloc_post(memkind_t kind, size_t nmemb, size_t size, void **result)
{
    if (WRAPPER_LIKELY(MemkindInitDone)) {
        LOG("\t calloc: size %zu\n", size);
    }
...
MEMKIND_WRAPPER_EXPORT void memkind_posix_memalign_post(memkind_t kind, void **memptr, size_t alignment, size_t size, int *err)
{
    if (WRAPPER_LIKELY(MemkindInitDone)) {
...
MEMKIND_WRAPPER_EXPORT void memkind_realloc_post(memkind_t *kind, void *ptr, size_t size, void **result)
{
    if (WRAPPER_LIKELY(MemkindInitDone)) {
        LOG("\t realloc: size %zu\n", size);
    }
