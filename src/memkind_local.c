...
static struct loc_numanodes_t memkind_balanced_loc_numanodes_g;
static pthread_once_t memkind_balanced_loc_numanodes_once_g = PTHREAD_ONCE_INIT;
...
static void memkind_balanced_loc_numanodes_init(void)
{
    struct loc_numanodes_t *g = &memkind_balanced_loc_numanodes_g;
    g->init_err = get_per_cpu_local_nodes_mask(&g->per_cpu_numa_nodes, MEM_ATTR_NONE);
}
...
static int memkind_balanced_loc_get_mbind_nodemask(
    struct memkind *kind, unsigned long *nodemask, unsigned long maxnode)
{
    struct loc_numanodes_t *g = &memkind_balanced_loc_numanodes_g;

    pthread_once(&memkind_balanced_loc_numanodes_once_g, memkind_balanced_loc_numanodes_init);
...
static int memkind_balanced_loc_finalize(memkind_t kind)
{
    local_kind_finalize(&memkind_balanced_loc_numanodes_g);
    return memkind_arena_finalize(kind);
}
...
static void memkind_balanced_loc_init_once(void)
{
    memkind_init(MEMKIND_BALANCED_LOCAL, true);
}
...
MEMKIND_EXPORT struct memkind_ops MEMKIND_BALANCED_LOCAL_OPS = {
    .create = memkind_arena_create,
    .destroy = memkind_default_destroy,
    .malloc = memkind_arena_malloc,
    .calloc = memkind_arena_calloc,
    ...
    .mbind = memkind_default_mbind,
    .get_mmap_flags = memkind_default_get_mmap_flags,
    .get_mbind_mode = memkind_preferred_many_get_mbind_mode,
    .get_mbind_flags = memkind_numa_balancing_get_mbind_flags,
    .get_mbind_nodemask = memkind_balanced_loc_get_mbind_nodemask,
    .get_arena = memkind_thread_get_arena,
    .init_once = memkind_balanced_loc_init_once,
    .malloc_usable_size = memkind_default_malloc_usable_size,
    .finalize = memkind_balanced_loc_finalize,
    .get_stat = memkind_arena_get_kind_stat,
    .defrag_reallocate = memkind_arena_defrag_reallocate
};
