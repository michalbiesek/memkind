...
void memtier_reset_size(unsigned kind_id)
{
    ...
    for (b = 0; b < THREAD_BUCKETS; ++b) {
        memkind_atomic_set(t_alloc_size[kind_id][b], 0);
    }
...
static inline void increment_alloc_size(unsigned kind_id, size_t size)
{
    ...
    unsigned b_id = t_hash_64();
    memkind_atomic_increment(t_alloc_size[kind_id][b_id], size);
    memkind_atomic_get(t_alloc_size[kind_id][b_id], total_size);
    if ((total_size >= FLUSH_THRESHOLD) || (total_size <= -FLUSH_THRESHOLD)) {
        long long update_size = memkind_atomic_exchange(t_alloc_size[kind_id][b_id], 0);
        memkind_atomic_increment(g_alloc_size[kind_id], update_size);
    }
}
...
static inline void decrement_alloc_size(unsigned kind_id, size_t size)
{
    long long total_size;
    unsigned b_id = t_hash_64();
    memkind_atomic_decrement(t_alloc_size[kind_id][b_id], size);
    memkind_atomic_get(t_alloc_size[kind_id][b_id], total_size);
    if ((total_size >= FLUSH_THRESHOLD) || (total_size <= -FLUSH_THRESHOLD)) {
        long long update_size = memkind_atomic_exchange(t_alloc_size[kind_id][b_id], 0);
        memkind_atomic_increment(g_alloc_size[kind_id], update_size);
...
MEMKIND_EXPORT size_t memtier_kind_allocated_size(memkind_t kind)
{
    ...
    for (b = 0; b < THREAD_BUCKETS; ++b) {
        long long update_size = memkind_atomic_exchange(t_alloc_size[kind->partition][b], 0);
        memkind_atomic_increment(g_alloc_size[kind->partition], update_size);
    }
    memkind_atomic_get(g_alloc_size[kind->partition], size_all);
    return size_all;
}
