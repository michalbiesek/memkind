...
static size_t alloc_global_size = 0;
...
MEMKIND_EXPORT void *memtier_tier_malloc(struct memtier_tier *tier, size_t size)
{
    ...
    alloc_global_size += jemk_malloc_usable_size(ptr);
    return ptr;
}
                                          ...
                                          size_t size)
{
    if (size == 0 && ptr != NULL) {
        ...
        alloc_global_size += jemk_malloc_usable_size(n_ptr);
        return n_ptr;
    } else {
        alloc_global_size -= jemk_malloc_usable_size(ptr);
                                               ...
                                               size_t size)
{
    ...
    alloc_global_size += jemk_malloc_usable_size(*memptr);
    return res;
}
