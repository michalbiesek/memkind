...
static size_t g_alloc_size[MEMKIND_MAX_KIND];
...
static inline void increment_alloc_size(unsigned kind_id, size_t size)
{
    ...
    if (memkind_atomic_increment(t_alloc_size[kind_id][bucket_id], size) >
        FLUSH_THRESHOLD) {
        size_t size_update =
            memkind_atomic_exchange(t_alloc_size[kind_id][bucket_id], 0);
        g_alloc_size[kind_id] += size_update;
    }
}
...
memtier_policy_static_threshold_get_kind(struct memtier_memory *memory)
{
    ...
    for (i = 1; i < memory->size; ++i) {
        if ((g_alloc_size[cfg[i].kind->partition] * cfg[i].kind_ratio) <
            g_alloc_size[cfg[0].kind->partition]) {
            dest_kind = i;
        }
    }
...
MEMKIND_EXPORT size_t memtier_kind_allocated_size(memkind_t kind)
{
    ...
    for (bucket_id = 0; bucket_id < THREAD_BUCKETS; ++bucket_id) {
        size = memkind_atomic_exchange(t_alloc_size[kind->partition][bucket_id],
                                       0);
        size_all += size;
    }
    g_alloc_size[kind->partition] = size_all;
    return size_all;
}
