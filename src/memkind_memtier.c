...
MEMKIND_ATOMIC __thread size_t alloc_size;
...
MEMKIND_EXPORT void *memtier_tier_malloc(struct memtier_tier *tier, size_t size)
{
    ...
    memkind_atomic_increment(alloc_size, jemk_malloc_usable_size(ptr));
    return ptr;
}
...
MEMKIND_EXPORT void memtier_free(void *ptr)
{
    ...
    if (!kind)
        return;
    memkind_atomic_decrement(alloc_size, jemk_malloc_usable_size(ptr));
