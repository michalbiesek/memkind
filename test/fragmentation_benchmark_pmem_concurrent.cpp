...
void release(int thread_id, char *source)
{
        if (!pmem_blocks.empty())
        {
                ...
                size_t evict_size = memkind_malloc_usable_size(MEMKIND_DAX_KMEM, pmem_str);
                memkind_free(MEMKIND_DAX_KMEM, pmem_str);
...
void allocate(int thread_id, size_t size)
{
    ...
    while((pmem_str = static_cast<char *>(memkind_malloc(MEMKIND_DAX_KMEM, size))) == nullptr)
    {
        release(thread_id, (char*)"failure_cleaner");
        retry_times++;
    }
    sprintf(pmem_str, "Hello world from pmem - pmem_str.\n");
    ...
    size_t usable_size = memkind_malloc_usable_size(MEMKIND_DAX_KMEM, pmem_str);
...
//        pmem_policy = std::stoul(argv[3]);
        allocate_thread_num = std::stoul(argv[1]);
        release_thread_num = std::stoul(argv[2]);
        pmem_size = std::stoull(argv[3]) * GB;
    }
